...
def _run_query_with_fallback(sql: str, params=None, fetch="one"):
    host, port, user, password, db_candidates = _get_db_cfg_candidates()

    if not password:
        return {
            "ok": False,
            "error": "DB_POSTGRESDB_PASSWORD não definida no ambiente",
            "data": None,
            "dbname": None,
        }
    ...
    for dbname in db_candidates:
        try:
            ...
            last_err = f"[db={dbname}] {_safe_error_to_str(e)}"
    ...
    try:
        ...
        last_err = _safe_error_to_str(e)

    return {"ok": False, "error": last_err, "data": None, "dbname": None}
...
def load_ml_full_coverage_kpi():
    ...
    try:
        ...
        if not res.get("ok"):
            return {"ok": False, "error": res.get("error"), "data": None}
        ...
        return {"ok": False, "error": _safe_error_to_str(e), "data": None}
...
st.caption(
    ...
    f"pass={'OK' if (os.getenv('DB_POSTGRESDB_PASSWORD') or os.getenv('POSTGRES_PASSWORD') or os.getenv('DB_PASSWORD')) else 'VAZIA'}"
)
...
def load_master_parquet(path: str) -> pd.DataFrame:
    if use_database_tables():
        ...
        # proteção extra para o plano free do Render
        if len(out) > 120000:
            if COL_DATA in out.columns:
        ...
        print(f"[APP] load_master_parquet via banco path={path} shape={out.shape}")
        return out
    ...
    print(f"[APP] load_master_parquet via arquivo path={path} shape={out.shape}")
    return out
...
if master is None or master.empty:
    ...
    st.error(f"Não consegui carregar a base principal ({origem}). Verifique `{ARQ_MASTER}` ou a tabela `dash_base_vendas_render`/`dash_base_vendas`.")
    st.stop()
